...
PIPELINE_REGISTRY.register_pipeline(
    ...
    pipeline_class=MyPipeline,
    pt_model=AutoModelForSequenceClassification,
    default=&amp;lcub;"pt": ("user/awesome_model", "abcdef")},
...
class PairClassificationPipeline(Pipeline):
    ...
    def postprocess(self, model_outputs):
        ...
        return &amp;lcub;"label": label, "score": score, "logits": logits}
```
...
repo = Repository("test-dynamic-pipeline", clone_from="&amp;lcub;your_username}/test-dynamic-pipeline")
...
`&amp;lcub;your_username}/test-dynamic-pipeline`. After that anyone can use it as long as they provide the option
`trust_remote_code=True`:

```py
from transformers import pipeline

classifier = pipeline(model="&amp;lcub;your_username}/test-dynamic-pipeline", trust_remote_code=True)
```
